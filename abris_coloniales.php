...
<?PHP
	...
	define('DEBUG', false);
	define('PS_SHOP_PATH', 'https://somesite.ru'); // fake site
	...
	$tmpimg = sys_get_temp_dir().DIRECTORY_SEPARATOR.'$imgc$.jpg';
	$currentpositionfile = sys_get_temp_dir().DIRECTORY_SEPARATOR.'coloniales1.pos';
	...
	try {
		...
		// кеш по годам
		$xml_props = $webService->get(array('resource' => 'product_feature_values', 'filter[id_feature]' => 3));
		...
		$xml_props = $webService->get(array('resource' => 'product_feature_values', 'filter[id_feature]' => 1));
		preg_match_all('|id="(\d*)|',$xml_props->children()->asXML(),$props);
		$props=$props[1];
		foreach($props as $prop){
			$xml_prop = $webService->get(array('resource' => 'product_feature_values/'.$prop));                       
	...
	while ($data = fgetcsv($fp, 16384, ';')) {
		...
		if (strlen ($str_ages))
			addfeature ($webService, $resources_product, $xml_feature, 10, $str_ages, 0, $age_cache);	// классы
		///////// ********************************************
		...
		// до сюда
			...
			if (strlen($_complect)) {
				$newFeature = $resources_product->associations->product_features->addChild('product_feature');
				$newFeature->addChild('id', 9);
				$newFeature->addChild('id_feature_value', 10);
			}
			...
			try {
				if(isset($years_cache[$val_year]))
					...
					$xml_feature->children()->children()->id_feature = 3;
					$xml_feature->children()->children()->custom = 0;
	...
	function calc_p ($abr_izd) {
		...
		if (strstr($abr_izd, 'М.: Академкнига/Учебник')) 	return array ('percent' => 25, 	'pub' => 3);
		if (strstr($abr_izd, 'М.: АСТ')) 			return array ('percent' => 25, 	'pub' => 4);
		if (strstr($abr_izd, 'М.: Баласс')) 			return array ('percent' => 25,	'pub' => 6);
		if (strstr($abr_izd, 'М.: Просвещение')) 		return array ('percent' => 25,	'pub' => 24);
		if (strstr($abr_izd, 'М.: Вентана-Граф')) 		return array ('percent' => 25,	'pub' => 8);
		if (strstr($abr_izd, 'М.: Вита-Пресс')) 		return array ('percent' => 25,	'pub' => 11);
		if (strstr($abr_izd, 'Самара: ИД ""Федоров""')) 	return array ('percent' => 25, 	'pub' => 34);
		if (strstr($abr_izd, 'Смоленск: Ассоциация XXI')) 	return array ('percent' => 25,	'pub' => 35);
		if (strstr($abr_izd, 'М.: Оникс')) 			return array ('percent' => 25, 	'pub' => 23);
		if (strstr($abr_izd, 'Обнинск: Титул')) 		return array ('percent' => 25,	'pub' => 31);
		if (strstr($abr_izd, 'М.: Версия')) 			return array ('percent' => 25,	'pub' => 10);
		if (strstr($abr_izd, 'М.: АСТ-Пресс')) 			return array ('percent' => 25, 	'pub' => 5); 
		if (strstr($abr_izd, 'М.: Март')) 			return array ('percent' => 25,	'pub' => 16);
		if (strstr($abr_izd, 'М.: МЦНМО')) 			return array ('percent' => 25, 	'pub' => 20);
		if (strstr($abr_izd, 'М.: Ювента')) 			return array ('percent' => 25, 	'pub' => 30);
		if (strstr($abr_izd, 'М.: Мнемозина')) 			return array ('percent' => 25,	'pub' => 18);
		if (strstr($abr_izd, 'М.: БИНОМ. Лаборатория знаний')) 	return array ('percent' => 25,	'pub' => 7);
		if (strstr($abr_izd, 'М.: Русское слово')) 		return array ('percent' => 25,	'pub' => 27);
		if (strstr($abr_izd, 'СПб.: Питер')) 			return array ('percent' => 25,	'pub' => 37);
		if (strstr($abr_izd, 'М.: Владос')) 			return array ('percent' => 25,	'pub' => 12);
		if (strstr($abr_izd, 'СПб.: Просвещение')) 		return array ('percent' => 25,	'pub' => 38);
		if (strstr($abr_izd, 'М.: Вербум-М')) 			return array ('percent' => 25,	'pub' => 9);
		if (strstr($abr_izd, 'М.: Академия')) 			return array ('percent' => 25,	'pub' => 2);
		if (strstr($abr_izd, 'М.: Интеллект-Центр')) 		return array ('percent' => 25,	'pub' => 17);
		if (strstr($abr_izd, 'М.: Олма Медиа Групп')) 		return array ('percent' => 25,	'pub' => 22);
		if (strstr($abr_izd, 'М.: Высшая школа')) 		return array ('percent' => 25,	'pub' => 13);
		if (strstr($abr_izd, 'М.: РОСТкнига')) 			return array ('percent' => 25, 	'pub' => 26);
		if (strstr($abr_izd, 'М.: Дрофа')) 			return array ('percent' => 25,	'pub' => 14);
		if (strstr($abr_izd, 'РнД.: Легион')) 			return array ('percent' => 25,	'pub' => 32);
		if (strstr($abr_izd, 'М.: Илекса')) 			return array ('percent' => 25, 	'pub' => 15);
		if (strstr($abr_izd, 'М.: Экзамен')) 			return array ('percent' => 25, 	'pub' => 29);
		if (strstr($abr_izd, 'М.: Самовар')) 			return array ('percent' => 25, 	'pub' => 28);
		if (strstr($abr_izd, 'Волгоград: Учитель')) 		return array ('percent' => 25,	'pub' => 1);
		if (strstr($abr_izd, 'М.: Мозаика - синтез')) 		return array ('percent' => 25, 	'pub' => 19);
		if (strstr($abr_izd, 'М.: Росмэн')) 			return array ('percent' => 25,	'pub' => 25);
		if (strstr($abr_izd, 'СПб.: Антология')) 		return array ('percent' => 25, 	'pub' => 36);
		if (strstr($abr_izd, 'М.: Национальное образование')) 	return array ('percent' => 25, 	'pub' => 21);
		if (strstr($abr_izd, 'РнД.: Феникс')) 			return array ('percent' => 25,	'pub' => 33);
	...
	function pereplet_id ($perepletstr) {
		if (strstr($perepletstr, "Digipack"))	return 1;
		if (strstr($perepletstr, "DVD"))		return 2;
		if (strstr($perepletstr, "Jewel"))	return 3;
		if (strstr($perepletstr, "Интегральный переплет"))	return 4;
		if (strstr($perepletstr, "Картон"))	return 5;
		if (strstr($perepletstr, "Обложка"))	return 6;
		if (strstr($perepletstr, "Папка"))	return 7;
		if (strstr($perepletstr, "Переплет"))	return 8;
	...
	function grp ($name, $abr_izd, $fp='') {
		...
		$subjects = array (
			array (	'id' => 13, 'pattern' => '(информат|компьют|программи|икт\b|excel|html|web|openoffice|flash|scratch|microsoft|иск.*интеллект)' ),
			array (	'id' => 7,  'pattern' => '(биологи[я|ч|и]|микробиолог)' ),
			array (	'id' => 17, 'pattern' => '(математик[а|и]|арифмет|математическ)' ),
			array (	'id' => 5,  'pattern' => 'алгебр[а|е|ы]' ),
			array (	'id' => 10, 'pattern' => 'геометри[я|ч|и|е]' ),
			array (	'id' => 35, 'pattern' => 'физи[ч|к]' ),
			array (	'id' => 36, 'pattern' => '(хими[я|ч|и|ю]|окисл.*восст)' ),
			array (	'id' => 11, 'pattern' => 'географи[я|ч|и]' ),
			array (	'id' => 14, 'pattern' => 'истори[я|ч|и]' ),
			array (	'id' => 22, 'pattern' => '(обществознани[я|е|и|ю]|право\b)' ),
			array (	'id' => 8,  'pattern' => 'естествознан' ),
			array (	'id' => 28, 'pattern' => 'тригонометри[я|ч|и|ю]' ),
			array (	'id' => 26, 'pattern' => 'словар(ь|ик)' ),
			array (	'id' => 25, 'pattern' => 'рус.*яз(ык|\.)' ),
			array (	'id' => 6,  'pattern' => '(англ.*яз(ык|\.)|english|английск)' ),
			array (	'id' => 31, 'pattern' => '(фр.*яз(ык|\.)|francais|французск)' ),
			array (	'id' => 20, 'pattern' => '(нем.*яз(ык|\.)|deutsch|немецк)' ),
			array (	'id' => 9,  'pattern' => 'язык' ),				// прочие языки
			array (	'id' => 18, 'pattern' => 'литератур[а|н|е]' ),			// литература после русского языка
			array (	'id' => 30, 'pattern' => 'физ.*культ' ),
			array (	'id' => 16, 'pattern' => 'культур(а|н\.)' ),
			array (	'id' => 24, 'pattern' => 'психолог' ),				// (ии|ическ|ия|о|а)
			array (	'id' => 19, 'pattern' => '(музык|сольфеджио|фортепиано|баян|ноты|аккордеон|синтезатор|гитар|скрипк|концерт)' ),
			array (	'id' => 33, 'pattern' => 'экономи[к|ч]' ),
			array (	'id' => 32, 'pattern' => 'черчени' ),
			array (	'id' => 27, 'pattern' => '(технология|по технологии)' ),
			array (	'id' => 23, 'pattern' => 'природоведени' ),
			array (	'id' => 15, 'pattern' => '(коррек|отклон)' ),
			array (	'id' => 4,  'pattern' => '(азбук|буквар)' ),
			array (	'id' => 29, 'pattern' => '(труд\b|труду\b|труд.обуч|трудовое обучение)' ),
			array (	'id' => 21, 'pattern' => '(обж\b|безопасност)' ),
			array (	'id' => 34, 'pattern' => 'энц' ),
			array (	'id' => 12, 'pattern' => '(изо\b|изобр)' ),
		);
